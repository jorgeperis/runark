  ...
  describe "GET /runs" do
    context "when authenticated" do
      ...
      it "filters by search query on the race name" do
        madrid = create(:run, race: create(:race, name: "Madrid Marathon"), user: user)
        valencia = create(:run, race: create(:race, name: "Valencia 10K"), user: user)
      ...
      it "filters by race" do
        race = create(:race)
        on_race = create(:run, race: race, user: user)
